...
class Workflow:
    ...
    def define_events(self, json_file=None):
        ...
        if json_file:
            ...
            for event in events:
                ...
                if event not in event_set:
                    ...
                    record = {'task_id': task['_id'], 'task_name': task['name'],
                              ...
                              'affected_objects': affected_objects, 'workflow_id': self.id}
                    self._events.add(self.dbs.add_to_database("Events", record))
            ...
            for task in tasks:
                ...
                events = task['input_events']+task['output_events']
                for event in events:
                    if event not in event_set:
                        event_set.add(event)
                        ...
                        record = {'task_id': task['_id'], 'task_name': task['name'],
                                  ...
                                  "name": event, 'description': description, 'workflow_id': self.id}
                        self._events.add(self.dbs.add_to_database("Events", record))
    ...
    def define_eca_rules(self, json_file=None):
        if json_file:
            ...
            for event_id in self._events:
                print(event_id)
                data = self.dbs.find_one_record("Events", {'_id': event_id})
                print("Define rules for the event ", data['name'])
                rule_ids = list()

                while True:
                    add = input("Press [Enter] to stop adding rules for an event "+data['name']+"\n")
                    if add:
                        count += 1
                        name = "R"+str(count)
                        condition_presence = input("Press [Enter] if condition not present else any key")
                        if condition_presence:
                            condition_id = self.define_condition(data, "C"+str(count))
                        else:
                            condition_id = None
                        ...
                        break
                print("Adding rules for the event", data['name'])
                self.dbs.update_record("Events", {"_id": event_id}, {"rules": rule_ids})
                print("Added")
        ...
        while True:
            ...
            if add:
                for event in self._events:
                    print(self.dbs.find_one_record("Events", {'_id': event})['name'])
    ...
    def _get_events(self, events):
        ...
        for event in events:
            ...
            rules = self.dbs.find_one_record("Events", event)['rules']
            rls = list()
            for rule in rules:
                rls.append(self.dbs.find_one_record("Rules", {'_id': rule})['name'])
    ...
    def _get_rules(self, rules):
        ...
        for rule in rules:
            r_data = dict()
            r_data['name'] = rule['name']
            r_data['action'] = self.dbs.find_one_record("Actions", {'_id': rule['action']})['name']
            condition = self.dbs.find_one_record("Conditions", {'_id': rule['condition']})
            if condition:
                r_data['condition'] = condition['name']
            else:
                r_data['condition'] = None
            rule_list.append(r_data)
        return rule_list
